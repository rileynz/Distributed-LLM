...
def plan_topology():
    ...
    for i, (s, e) in enumerate(splits):
        ...
        host_str = ask(f"  IP/host for node {i} (use 127.0.0.1 if on this machine, "
                       f"or the node's LAN IP)", "127.0.0.1")
...
def configure_firewall(nodes, registry_host, registry_port):
    ...
    ports = [n["port"] for n in nodes] + [n["port"] + 10000 for n in nodes]
    ports.append(registry_port)
    ...
    if IS_WINDOWS:
        ...
        info(f"Ports needed: {local_ports + [registry_port]}")
        choice = ask("Add Windows Firewall rules automatically? [Y/n]", "Y").upper()
        if choice == "Y":
            _add_windows_rules(local_ports + [registry_port])
        else:
        ...
        info(f"Ports to allow inbound TCP: {local_ports + [registry_port]}")
        if platform.system() == "Linux":
            choice = ask("Try to add ufw rules? (only if ufw is active) [y/N]", "N").upper()
            if choice == "Y":
                for port in local_ports + [registry_port]:
                    try:
...
def print_summary(nodes, registry_host, registry_port):
    ...
    print(f"  ① Registry:")
    print(f"      python run.py  → choose 1, port {registry_port}\n")
    for i, node in enumerate(nodes):
        print(f"  {'②③④⑤⑥⑦⑧⑨'[i] if i < 8 else str(i+2)} Node {i} (layers {node['start']}-{node['end']-1}):")
        print(f"      python run.py  → choose 2")
    ...
    print(f"      python run.py  → choose 3")
    print(f"      registry URL: {registry_url}\n")
    print(f"  To download a real AI model after everything is running:")
    print(f"      python run.py  → choose 4 (Models)\n")
    print(f"  Your LAN IP (for other machines): {_get_local_ip()}")
    print()
