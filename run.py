...
def run_node():
    ...
    if not Path(CHECKPOINT_PATH).exists():
        err("model.pt not found — run create_checkpoint.py first.")
        return
    ...
    cmd = [sys.executable, "node/server.py",
           ...
           "--registry", registry_url, "--label", label]
    try:
        subprocess.run(cmd)
    except KeyboardInterrupt:
        pass
...
def run_client():
    ...
    if st:
        ...
        alive = sorted([n for n in st["nodes"] if n["alive"]],
...
def main():
    ...
    while True:
        ...
        try:
            choice = input(
                f"  {cyan('→')} {bold('Choose')} {gray('[1-5 / R / Q]')}: "
            ).strip().upper()
        except (EOFError, KeyboardInterrupt):
            print(f"\n\n  {yellow('Goodbye.')}\n")
            sys.exit(0)
