...
- Fixed GitHub Actions builds when web uploads remove Linux executable bits.
- Fixed Windows builders selecting a different system Python than
  `actions/setup-python`, and ensured dependencies install before tests.
- Added an isolated Windows CI environment, a verified `cryptography` import,
  and a clear guard against accidentally running nested or outdated files.
